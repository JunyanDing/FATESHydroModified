         ...
         if ( laisun_lsl + laisha_lsl > 0._r8 ) then 
           ...
           do  sunsha = 1,2      
              ...
              do while(loop_continue)                 
                 ...
                 call quadratic_f (aquad, bquad, cquad, r1, r2)
                 gs_mol = max(r1,r2)                 
                 ! Junyan added codes below
                 cquadw = -gb_mol*((leaf_co2_ppress*(bbb/btran) + &
                                  bb_slope(ft)*anet*can_press * ceair/ veg_esat ))*btran
                 call quadratic_f (aquad, bquad, cquadw, rw1, rw2)  
                 gsw_mol = max(rw1,rw2)
              ...
              gs = gs_mol / cf
	          gsw = gsw_mol / cf  ! Junyan's addition,not used though
              ...
              if(sunsha == 1)then !sunlit       
                 ...
                 if (hlm_use_planthydro.eq.itrue ) then
                    gstoma  = gstoma + 1._r8/(min(1._r8/gsw, rsmax0)) * f_sun_lsl
                 else
                 ...
                 !if (hlm_use_planthydro.eq.itrue ) then
                 !  gstoma  = gstoma + &
                 !      1._r8/(min(1._r8/gsw, rsmax0)) * (1.0_r8-f_sun_lsl) 
                 !else
                 gstoma  = gstoma + &
           ...
           ! when hydro is on, further scale stoma conductance by leaf water pontetial to compansate the esat in CLM , Junyan
	      if (hlm_use_planthydro.eq.itrue ) then 
             rstoma_out = 1._r8/(gstoma*max(1.0_r8,(thl/thsl)**0.5_r8))  
          else
