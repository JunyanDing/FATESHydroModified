...
contains 
 ...
 subroutine initTreeHydStates(site_p, cc_p, bc_in)
  ...
  subroutine UpdateTreeHydrLenVolCond(ccohort,nlevsoi_hyd,bc_in)
      ...
      logical :: Junyan        = .true.              ! indicate the part modified by Junyan, set to false to use original codes

      real(r8) :: sum_rootfr                         ! sum of root fraction for normalization

      real(r8) :: root_fraction(1:nlevsoi_hyd)
      ...
      if(leaf_c > 0._r8) then
         ...
         if(nlevsoi_hyd == 1) then
          ...
          ! code below are modified by Junyan to ignore the absorbing roots at top soil layer
          if(Junyan == .false.) then
            ! original version
            do j=1,nlevsoi_hyd
               if(j == 1) then
              ...
              do j=1,nlevsoi_hyd
                 if(j == 1) then
                     ...
                     root_fraction(j) = 0.01_r8
                
                 else
                    root_fraction(j) = zeng2001_crootfr(roota, rootb, bc_in%zi_sisl(j)) - &
                     zeng2001_crootfr(roota, rootb, bc_in%zi_sisl(j-1))
                 end if
                 sum_rootfr = sum_rootfr + root_fraction(j)
                  
                end do
                
                ! Normalize the root profile
                ccohort_hydr%l_aroot_layer(1:nlevsoi_hyd) = (root_fraction(1:nlevsoi_hyd)/sum_rootfr)*ccohort_hydr%l_aroot_tot
                ccohort_hydr%v_aroot_layer(1:nlevsoi_hyd) = (root_fraction(1:nlevsoi_hyd)/sum_rootfr)*ccohort_hydr%v_aroot_tot
...
!                call swcVG_psi_from_satfrac(s_shell_init(j,k),alpha_VG(c,j),n_VG(c,j),m_VG(c,j),l_VG(c,j),psi_shell_init(j,k))
  ...
  subroutine hydraulics_bc ( nsites, sites,bc_in,bc_out,dtime )
     ...
     integer  :: maxiter        = 6            ! maximum iterations for timestep reduction                       [-]   Junyan changed maxiter from 1 to 5
...
!                     bc_in(s)%h2o_liq_sisl(bc_in(s)%nlevsoil)/(bc_in(s)%dz_sisl(bc_in(s)%nlevsoil)*denh2o))
  ...
  subroutine Hydraulics_1DSolve(cc_p, ft, z_node, v_node, ths_node, thr_node, kmax_bound, &
    ...
    real(r8) , intent(in)    :: qtop                   ! total transpiration flux from canopy                                    [kgh2o indiv-1 s-1]
